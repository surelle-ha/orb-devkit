<template>
  <div class="devkit-root pb-28" style="touch-action:pan-y;">
    ...
    <!-- ══ VAULT LOCK BANNER (shared with password manager) ══ -->
    <div v-if="!envUnlocked" class="flex flex-col items-center justify-center min-h-[80dvh] px-8 gap-6">
      <div class="relative">
        <div class="w-20 h-20 rounded-3xl flex items-center justify-center"
          :style="{ background: accent + '18', border: `1px solid ${accent}33` }">
          <KeyRound :size="32" :style="{ color: accent }" :stroke-width="1.5" />
        </div>
      ...
      <div class="text-center">
        <p class="text-[10px] font-mono text-zinc-600 tracking-[0.25em] uppercase mb-1">orb devkit / env_manager</p>
        <h1 class="text-[22px] font-black text-zinc-50 tracking-tight font-mono">
          <span :style="{ color: accent }">›</span> .env vault
        </h1>
        <p class="text-[12px] font-mono text-zinc-500 mt-2 leading-relaxed">
          Shared vault password required.<br>Same password as your credential vault.
        </p>
      </div>

      <div class="w-full max-w-[340px] flex flex-col gap-3">
        <div class="flex items-center gap-2 rounded-2xl px-4 py-3.5"
          :class="envUnlockError ? 'border-2 border-rose-500/60 bg-rose-950/20' : ''"
          :style="!envUnlockError ? { background:'rgba(255,255,255,0.05)', border:`1px solid rgba(255,255,255,0.1)` } : {}">
          <Lock :size="15" :class="envUnlockError ? 'text-rose-400' : 'text-zinc-600'" :stroke-width="2" class="flex-shrink-0" />
          <input
            v-model="masterInput"
            :type="showMasterPwd ? 'text' : 'password'"
            placeholder="Vault master password…"
            autocomplete="current-password"
            @keydown.enter="unlockEnv"
            class="flex-1 bg-transparent text-[14px] font-mono text-zinc-100 placeholder:text-zinc-700 outline-none"
          />
          <button @click="showMasterPwd = !showMasterPwd" class="text-zinc-700 active:text-zinc-400 flex-shrink-0">
            <Eye v-if="!showMasterPwd" :size="14" :stroke-width="2" />
            <EyeOff v-else :size="14" :stroke-width="2" />
          </button>
        </div>
        <p v-if="envUnlockError" class="text-[12px] font-bold text-rose-400 text-center -mt-1">{{ envUnlockError }}</p>

        <button @click="unlockEnv"
        ...
        <p v-if="!hasMasterSet" class="text-[11px] font-mono text-zinc-600 text-center leading-relaxed">
          ...
          <span class="text-zinc-500">(shared with Password Manager)</span>
        </p>

        <button v-if="!hasMasterSet" @click="unlockEnv"
    ...
    <!-- ══ UNLOCKED: FULL ENV MANAGER ══ -->
    <template v-if="envUnlocked">
        ...
        <button v-for="proj in projects" :key="proj.id" @click="activeProjectId = proj.id"
          ...
          <div class="flex items-center gap-3 px-4 py-4">
            ...
            <div class="flex flex-col items-end gap-1.5 flex-shrink-0">
              ...
              <span class="text-[9px] font-mono text-zinc-600">{{ totalVarCount(proj) }} vars</span>
            </div>
          </div>
        </button>
      </div>
        ...
        <button v-for="inst in activeProject.instances" :key="inst.id" @click="activeInstanceId = inst.id"
          ...
          <div class="flex items-center gap-3 px-4 py-4">
            <div class="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0"
              ...
              <component :is="instanceIcon(inst.type)" :size="20" :style="{ color: instanceColor(inst.type) }" :stroke-width="1.8" />
            </div>
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
        ...
        <div class="w-full max-w-[430px] rounded-t-[28px] border-t overflow-hidden"
          ...
          <div class="flex flex-col gap-3 px-5 pt-4">
              ...
              <div class="grid grid-cols-3 gap-2">
                <button v-for="t in instanceTypes" :key="t.value" @click="selectInstanceType(t.value)"
                  ...
                  <component :is="instanceIcon(t.value)" :size="18"
                    :style="{ color: instanceForm.type === t.value && !instanceForm.isCustomType ? instanceColor(t.value) : '#52525b' }"
                    :stroke-width="1.8" />
                ...
                <!-- Custom type button -->
                <button @click="selectCustomType"
                  class="flex flex-col items-center gap-1.5 py-3 rounded-xl transition-all active:scale-95"
                  :style="instanceForm.isCustomType
                    ? { background: accent + '18', border: `1px solid ${accent}44` }
                    : { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }">
                  <Pencil :size="18"
                    :style="{ color: instanceForm.isCustomType ? accent : '#52525b' }"
                    :stroke-width="1.8" />
                  <span class="text-[10px] font-mono font-bold"
                    :style="{ color: instanceForm.isCustomType ? accent : '#52525b' }">
                    custom
            ...
            <Transition name="dropdown">
              <div v-if="instanceForm.isCustomType">
                <p class="text-[9px] font-mono font-bold text-zinc-600 uppercase tracking-widest mb-1.5 px-1">CUSTOM TYPE NAME</p>
                <input v-model="instanceForm.customTypeName"
                  placeholder="e.g. canary, feature, hotfix…"
                  maxlength="20"
                  class="w-full rounded-xl px-4 py-3 text-[13px] font-mono font-bold text-zinc-100 placeholder:text-zinc-700 outline-none"
                  style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);"
                  @input="instanceForm.customTypeName = instanceForm.customTypeName.toLowerCase().replace(/[^a-z0-9-_]/g, '')" />
                <p class="text-[9px] font-mono text-zinc-700 mt-1 px-1">lowercase, letters/numbers/hyphens only</p>
              </div>
            </Transition>

            <!-- Name -->
            <div>
              <p class="text-[9px] font-mono font-bold text-zinc-600 uppercase tracking-widest mb-1.5 px-1">DISPLAY NAME</p>
              <input v-model="instanceForm.name" :placeholder="instanceForm.isCustomType ? (instanceForm.customTypeName || 'my-env') : (instanceForm.type || 'dev')"
                class="w-full rounded-xl px-4 py-3 text-[14px] font-mono font-bold text-zinc-100 placeholder:text-zinc-700 outline-none"
                style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);" />
            </div>
            ...
            <div class="flex gap-2 mt-1">
              ...
              <button @click="saveInstance" :disabled="!canSaveInstance"
                class="flex-1 py-3.5 rounded-xl text-[14px] font-black font-mono active:scale-[0.98] transition-all"
                :style="canSaveInstance
                  ? { background: accent, color: '#fff', boxShadow: `0 6px 20px ${accent}44` }
                  : { background: 'rgba(255,255,255,0.05)', color: '#3f3f46', border: '1px solid rgba(255,255,255,0.06)' }">
          ...
          <div class="flex flex-col gap-3 px-5 pt-4">
            ...
            <button @click="varForm.secret = !varForm.secret"
              ...
              <div class="flex-1 text-left">
                <p class="text-[12px] font-mono font-bold" :class="varForm.secret ? 'text-amber-300' : 'text-zinc-500'">Mark as secret</p>
              </div>
              <div :class="['w-10 h-5 rounded-full transition-all relative flex-shrink-0', varForm.secret ? 'bg-amber-500' : 'bg-zinc-800']">
                <div :class="['absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all', varForm.secret ? 'left-5' : 'left-0.5']"></div>
              </div>
            </button>
            <div class="flex gap-2 mt-1">
...
import {
  ...
  Server, Globe, HardDrive, Shield, KeyRound,
...
// ══════════════════════════════════════════════════════════
// VAULT LOCK — shared with passwords.vue (orb_vault_master_v1)
// ══════════════════════════════════════════════════════════
...
async function unlockEnv() {
  if (!masterInput.value.trim()) { envUnlockError.value = 'Enter a password'; return }
  const hash   = await sha256(masterInput.value.trim())
  const stored = localStorage.getItem(VAULT_MASTER_KEY)

  if (!stored) {
    // First time — set master (shared with vault)
    localStorage.setItem(VAULT_MASTER_KEY, hash)
    hasMasterSet.value = true
    envUnlocked.value  = true
    envUnlockError.value = ''
    orbLog('Vault master password set via ENV page')
  } else if (hash === stored) {
    envUnlocked.value  = true
    envUnlockError.value = ''
    masterInput.value  = ''
    orbLog('ENV vault unlocked')
  } else {
    envUnlockError.value = 'Incorrect password — same as your vault password'
    masterInput.value = ''
  }
}
...
interface EnvVar {
  id: string; key: string; value: string; type: string; secret: boolean
}
interface EnvInstance {
  id: string; name: string; type: string; vars: EnvVar[]
}
interface EnvProject {
  id: string; name: string; description: string; icon: string; color: string
  instances: EnvInstance[]; createdAt: string
}
...
function loadProjects(): EnvProject[] {
  try { const r = localStorage.getItem(ENV_PROJECTS_KEY); if (r) return JSON.parse(r) } catch {}
  return []
}
...
function goHome()      { activeProjectId.value = null; activeInstanceId.value = null }
function goToProject() { activeInstanceId.value = null }
...
function instanceColor(type: string): string {
  const map: Record<string, string> = {
    dev: '#34d399', qa: '#60a5fa', staging: '#fb923c',
    prod: '#f87171', local: '#a78bfa', ci: '#fbbf24',
  }
  // Custom types get the accent color
  return map[type] ?? accent.value
}

function instanceIcon(type: string) {
  const map: Record<string, any> = {
    dev: FlaskConical, qa: Shield, staging: Server,
    prod: Globe, local: HardDrive, ci: Layers,
  }
  // Custom types get Pencil icon
  return map[type] ?? Pencil
}
...
const projectForm = reactive({ name: '', description: '', icon: '🚀', color: '#8b5cf6' })

function onAddClick() {
  if (!activeProject.value)       showProjectSheet.value = true
  else if (!activeInstance.value) { showInstanceSheet.value = true; instanceForm.projectId = activeProject.value.id }
  else                            openAddVar()
}

function editProject(proj: EnvProject) {
  editProjectTarget.value = proj
  projectForm.name = proj.name; projectForm.description = proj.description
  projectForm.icon = proj.icon; projectForm.color = proj.color
  showProjectSheet.value = true
}

function closeProjectSheet() {
  showProjectSheet.value = false; editProjectTarget.value = null
...
function saveProject() {
  ...
  if (editProjectTarget.value) {
    ...
    if (p) { p.name = projectForm.name.trim(); p.description = projectForm.description.trim(); p.icon = projectForm.icon; p.color = projectForm.color }
    orbLog(`Project updated: ${projectForm.name}`)
  } else {
    projects.value.push({ id: uid(), name: projectForm.name.trim(), description: projectForm.description.trim(), icon: projectForm.icon, color: projectForm.color, instances: [], createdAt: new Date().toISOString() })
    orbLog(`Project created: ${projectForm.name}`)
  }
  saveProjects(); closeProjectSheet(); showToast(editProjectTarget.value ? 'project_updated' : 'project_created')
}

function confirmDeleteProject(id: string) {
  projects.value = projects.value.filter(p => p.id !== id)
  if (activeProjectId.value === id) activeProjectId.value = null
  saveProjects(); showToast('project_deleted'); closeProjectSheet()
}
function deleteProjectConfirmed() { if (!editProjectTarget.value) return; confirmDeleteProject(editProjectTarget.value.id) }

// ══════════════════════════════════════════════════════════
// INSTANCE CRUD — with custom type support
// ══════════════════════════════════════════════════════════
...
const canSaveInstance = computed(() => {
  if (instanceForm.isCustomType) {
    return instanceForm.customTypeName.trim().length > 0 && instanceForm.name.trim().length > 0
  }
  return instanceForm.name.trim().length > 0
})

function selectInstanceType(value: string) {
  instanceForm.type         = value
  instanceForm.isCustomType = false
  instanceForm.customTypeName = ''
}

function selectCustomType() {
  instanceForm.isCustomType = true
  instanceForm.type         = 'custom'
}

function editInstance(inst: EnvInstance) {
  editInstanceTarget.value = inst
  // Detect if it's a custom type
  const isPreset = instanceTypes.some(t => t.value === inst.type)
  instanceForm.name           = inst.name
  instanceForm.type           = inst.type
  instanceForm.isCustomType   = !isPreset
  instanceForm.customTypeName = !isPreset ? inst.type : ''
  instanceForm.projectId      = activeProjectId.value ?? ''
  showInstanceSheet.value     = true
}

function closeInstanceSheet() {
  showInstanceSheet.value = false; editInstanceTarget.value = null
  Object.assign(instanceForm, { projectId: '', name: '', type: 'dev', isCustomType: false, customTypeName: '' })
}

function saveInstance() {
  if (!canSaveInstance.value) return
  const proj = projects.value.find(p => p.id === (instanceForm.projectId || activeProjectId.value))
  if (!proj) return

  // Resolve final type string
  const finalType = instanceForm.isCustomType ? instanceForm.customTypeName.trim() : instanceForm.type

  if (editInstanceTarget.value) {
    const inst = proj.instances.find(i => i.id === editInstanceTarget.value!.id)
    if (inst) { inst.name = instanceForm.name.trim(); inst.type = finalType }
    orbLog(`Instance updated: ${instanceForm.name}`)
  } else {
    proj.instances.push({ id: uid(), name: instanceForm.name.trim(), type: finalType, vars: [] })
    orbLog(`Instance created: ${instanceForm.name} (${finalType}) in ${proj.name}`)
  }
  saveProjects(); closeInstanceSheet(); showToast(editInstanceTarget.value ? 'environment_updated' : 'environment_added')
}
...
function deleteInstance(id: string) {
  ...
  saveProjects(); closeInstanceSheet(); showToast('environment_deleted')
}
...
const varForm = reactive({ key: '', value: '', type: 'string', secret: false, revealInForm: false })
...
function editVar(v: EnvVar) {
  editVarTarget.value = v; varForm.key = v.key; varForm.value = v.value
  varForm.type = v.type; varForm.secret = v.secret; varForm.revealInForm = false
  showVarSheet.value = true
}

function closeVarSheet() { showVarSheet.value = false; editVarTarget.value = null }
...
async function saveVar() {
  ...
  if (editVarTarget.value) {
    ...
    if (v) Object.assign(v, { key: varForm.key, value: varForm.value, type: varForm.type, secret: varForm.secret })
    orbLog(`ENV updated: ${varForm.key}`)
  } else {
    inst.vars.push({ id: uid(), key: varForm.key, value: varForm.value, type: varForm.type, secret: varForm.secret })
    orbLog(`ENV added: ${varForm.key}`)
  }
  saveProjects()
  if (daemonConnected.value && proj) {
    await syncSingleEnv(proj.name, inst.name, inst.vars).catch(e => orbLog(`Daemon ENV sync failed: ${e.message}`, 'warn'))
  }
  closeVarSheet(); showToast(editVarTarget.value ? 'var_updated' : 'var_added')
}
...
async function deleteVar(id: string) {
  ...
  closeVarSheet(); showToast(`deleted: ${v?.key}`); orbLog(`ENV deleted: ${v?.key}`)
}
...
.dropdown-enter-to, .dropdown-leave-from { opacity: 1; max-height: 120px; }
</style>