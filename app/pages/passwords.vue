<template>
  <div class="devkit-root pb-28" style="touch-action:pan-y;">
    ...
    <!-- Vault lock banner -->
    <div v-if="!vaultUnlocked" class="mx-4 mb-4 rounded-2xl overflow-hidden"
      :style="{ background: accent + '08', border: `1px solid ${accent}22` }">
      <div class="flex flex-col items-center gap-4 px-6 py-6">
        <div class="w-16 h-16 rounded-2xl flex items-center justify-center"
          :style="{ background: accent + '18', border: `1px solid ${accent}33` }">
          <ShieldCheck :size="28" :style="{ color: accent }" :stroke-width="1.8" />
        </div>
        ...
        <div class="w-full flex gap-2">
          <div class="flex-1 flex items-center gap-2 rounded-2xl px-4 py-3"
            ...
            <Lock :size="15" class="text-zinc-600 flex-shrink-0" :stroke-width="2" />
            <input v-model="masterInput" :type="showMaster ? 'text' : 'password'"
              placeholder="Master password…" @keydown.enter="unlockVault"
          ...
          <button @click="unlockVault"
            class="px-4 rounded-2xl text-[13px] font-black font-mono active:scale-95 transition-all"
            :style="{ background: accent, color: '#fff', boxShadow: `0 4px 16px ${accent}44` }">
            Unlock
          </button>
        </div>
    ...
    <template v-if="vaultUnlocked">
      ...
      <div class="flex gap-2 px-4 mb-4 overflow-x-auto scrollbar-hide">
        <button v-for="cat in ['all', ...vaultCategories]" :key="cat"
          @click="activeCategory = cat"
          class="flex-shrink-0 px-3 py-1.5 rounded-xl text-[11px] font-mono font-bold transition-all"
  ...
  <Teleport to="body">
    <Transition name="sheet">
      <div v-if="showSheet && vaultUnlocked"
            ...
            <div>
              ...
              <div class="flex gap-2">
                <div class="flex-1 flex items-center gap-2 rounded-xl px-4 py-3"
                  style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);">
                  <input v-model="form.password" :type="showFormPwd ? 'text' : 'password'" placeholder="••••••••••••" autocomplete="new-password"
...
async function unlockVault() {
  if (!masterInput.value.trim()) return
