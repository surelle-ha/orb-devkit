      ...
      <button @click="navigate('env')"
        ...
        <div class="flex-shrink-0 flex flex-col items-end gap-1">
          <div class="flex items-center gap-1.5 px-2 py-1 rounded-lg"
            :style="{ background: daemonConnected ? 'rgba(52,211,153,0.1)' : 'rgba(239,68,68,0.08)', border: `1px solid ${daemonConnected ? 'rgba(52,211,153,0.2)' : 'rgba(239,68,68,0.15)'}` }">
            <div class="w-1.5 h-1.5 rounded-full" :class="daemonConnected ? 'bg-emerald-400' : 'bg-rose-500'"></div>
            <span class="text-[9px] font-mono font-bold" :class="daemonConnected ? 'text-emerald-400' : 'text-rose-400'">
      ...
      <div v-if="showDevConfirm" class="fixed inset-0 z-[400] flex items-end justify-center"
        ...
        <div class="w-full max-w-[430px] rounded-t-[28px] border-t pb-10 px-5 pt-5"
          ...
          <p class="text-[13px] text-center leading-relaxed mb-6 text-zinc-500">
            {{ devMode
              ? 'This will disable extended logging, debug overlays, and disconnect from the daemon.'
              : 'This enables hot reload, extended logs, debug overlays, and daemon connection.' }}
          </p>
...
function goToDevice() {
  ...
  if (first) navigate('devices', { deviceId: first.id })
...
watch([daemonConnected, devMode, balanceStyle], ([connected, devOn, style]) => {
  if (twinOrbTimer) clearTimeout(twinOrbTimer)
  if (connected && devOn && style === 'supreme') {
    twinOrbTimer = setTimeout(() => { showTwinOrb.value = true }, 1500)
  } else {
...
const orbStatusLine = computed(() => {
  if (devMode.value && daemonConnected.value) return `daemon · live · ${devSessionTime.value}`
  if (devMode.value) return `dev_mode · active · ${devSessionTime.value}`
  return 'tap to start dev_mode'
})

const coreMetrics = computed(() => [
  {
    label: 'daemon',
    value: daemonConnected.value ? 'LIVE' : 'DOWN',
    sub: daemonConnected.value ? `${daemonLatency.value != null ? daemonLatency.value + 'ms' : 'linked'}` : 'not linked',
    color: daemonConnected.value ? '#34d399' : '#ef4444',
  },
  {
    label: 'devices',
...
@keyframes tcp-particle-move {
  ...
  50%  { transform:translateX(-30px) translateY(-20px); opacity:.3; }
