<template>
  <div class="bg-slate-100 dark:bg-zinc-950 pb-28" style="touch-action:pan-y;">
    ...
    <div
      class="mx-4 mb-4 rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-slate-200/60 dark:border-zinc-800/60 shadow-sm">
        ...
        <button @click="accentOpen = !accentOpen"
          ...
          <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
            ...
            <div class="grid grid-cols-2 gap-1 p-1.5">
              <div v-for="col in ACCENT_COLORS.slice(0, 4)" :key="col.hex" class="w-2.5 h-2.5 rounded-full"
          ...
          <ChevronDown :size="17"
            :class="['text-slate-400 dark:text-zinc-500 transition-transform duration-200 flex-shrink-0', accentOpen ? 'rotate-180' : '']"
            :stroke-width="2" />
        </button>
        <Transition name="dropdown">
          <div v-if="accentOpen" class="px-4 pb-4 pt-1 border-t border-slate-100 dark:border-zinc-800/60">
            <div class="grid grid-cols-6 gap-3 mb-3">
              <button v-for="col in ACCENT_COLORS" :key="col.hex" @click="setAccent(col.hex)"
                ...
                <div class="w-10 h-10 rounded-full transition-all duration-200 flex items-center justify-center"
                  ...
                  <svg v-if="accent === col.hex" viewBox="0 0 12 12" width="13" height="13">
                    <polyline points="2,6 5,9 10,3" fill="none" stroke="white" stroke-width="2" stroke-linecap="round"
                      stroke-linejoin="round" />
                  </svg>
                </div>
                <span
                  :class="['text-[9px] font-bold truncate w-full text-center leading-none', accent === col.hex ? 'text-slate-700 dark:text-zinc-200' : 'text-slate-400 dark:text-zinc-600']">{{
                    col.label }}</span>
              </button>
            </div>
            <div class="flex items-center gap-2.5 pt-2.5 border-t border-slate-100 dark:border-zinc-800">
              ...
              <button @click="applyCustom"
                ...
                :style="{ background: `${accent}20`, color: accent }">Apply</button>
            </div>
          </div>
        </Transition>
      </div>
      ...
      <div class="border-t border-slate-100 dark:border-zinc-800/60 px-4 py-3.5">
        ...
        <div class="grid grid-cols-2 gap-2.5">
          <button v-for="style in cardStyles" :key="style.value" @click="saveSettings({ balanceStyle: style.value })"
            ...
            <div class="w-full rounded-xl overflow-hidden mb-2 relative" style="height:64px;"
              ...
              <div class="absolute flex items-center justify-center" :style="style.orbStyle(accent)">
                ...
                <div class="absolute rounded-full pointer-events-none"
                  :style="{ inset: '-8px', background: `radial-gradient(circle, ${accent}44 0%, transparent 70%)`, filter: 'blur(5px)', borderRadius: '50%' }">
                </div>
                <!-- Orbit ring -->
            ...
            <p
              :class="['text-[12px] font-black text-center', settings.balanceStyle === style.value ? 'text-violet-600 dark:text-violet-300' : 'text-slate-500 dark:text-zinc-400']">
              {{ style.label }}</p>
            <p class="text-[9px] text-slate-400 dark:text-zinc-600 text-center mt-0.5">{{ style.sub }}</p>
            <div v-if="settings.balanceStyle === style.value"
    ...
    <div
      ...
      <div class="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-zinc-800/60">
        ...
        <button @click="toggleIdleLock" class="w-12 h-6 rounded-full transition-all relative flex-shrink-0"
          ...
          <div
            :class="['absolute top-0.5 w-5 h-5 rounded-full bg-white shadow-sm transition-all', settings.idleLockEnabled ? 'left-6' : 'left-0.5']">
          </div>
        </button>
      </div>
      <Transition name="dropdown">
        <div v-if="settings.idleLockEnabled" class="px-4 py-4 border-b border-slate-100 dark:border-zinc-800/60">
          <p class="text-[12px] font-bold text-slate-500 dark:text-zinc-400 mb-3">
            Lock after <span :style="{ color: accent }">{{ settings.idleLockMinutes }} min{{ settings.idleLockMinutes >
              1 ? 's' : '' }}</span> of inactivity
          </p>
      ...
      <!-- App PIN -->
      <div class="flex items-center gap-3 px-4 py-3.5"
        :class="pinEnabled ? 'border-b border-slate-100 dark:border-zinc-800/60' : ''">
        <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
          :style="{ background: `${accent}18` }">
    ...
    <!-- DATA -->
    <div class="px-5 pb-2">
      <h3 class="text-[11px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Data</h3>
    </div>
    <div
      class="mx-4 mb-4 rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-slate-200/60 dark:border-zinc-800/60 shadow-sm">
    ...
    <div class="px-5 pb-2">
      <h3 class="text-[11px] font-bold text-rose-400/70 dark:text-rose-500/70 uppercase tracking-widest">Advanced</h3>
    </div>
    <div
      class="mx-4 mb-4 rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-slate-200/60 dark:border-zinc-800/60 shadow-sm">
      ...
      <button @click="navigate('developer')"
        class="w-full flex items-center gap-3 px-4 py-3.5 border-b border-slate-100 dark:border-zinc-800/60 active:bg-rose-50 dark:active:bg-rose-950/20 transition-colors">
        <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[11px] text-slate-400 dark:text-zinc-500 mt-0.5">Logs, diagnostics, reset &amp; perf monitor
          </p>
        </div>
        <ChevronRight :size="17" class="text-slate-300 dark:text-zinc-700 flex-shrink-0" :stroke-width="2" />
      </button>

      <!-- Reset Daemon Connection -->
      <button @click="showResetDaemon = true"
        ...
        <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
          ...
          <WifiOff :size="19" class="text-orange-400" :stroke-width="1.8" />
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-[14px] font-bold text-orange-400">Reset Daemon Pairing</p>
          <p class="text-[11px] text-slate-400 dark:text-zinc-500 mt-0.5">Clears saved daemon connection</p>
        </div>
        <ChevronRight :size="17" class="text-slate-300 dark:text-zinc-700 flex-shrink-0" :stroke-width="2" />
      </button>
    </div>
    ...
    <!-- DANGER ZONE -->
    <div class="px-5 pb-2">
      <h3 class="text-[11px] font-bold text-rose-500 dark:text-rose-400 uppercase tracking-widest">Danger Zone</h3>
    </div>
    <div
      class="mx-4 mb-4 rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-rose-200/60 dark:border-rose-900/40 shadow-sm">
    ...
    <div
      class="mx-4 rounded-2xl bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-slate-200/60 dark:border-zinc-800/60 shadow-sm px-4 py-4 flex items-center gap-3">
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showResetDaemon" class="fixed inset-0 z-[300] flex items-end justify-center"
        style="background:rgba(0,0,0,0.65);backdrop-filter:blur(14px)" @click.self="showResetDaemon = false">
        <div class="w-full max-w-[430px] rounded-t-[28px] border-t pb-10 px-5 pt-5 bg-[#0e0b1e] border-orange-900/40">
          <div class="w-10 h-1 rounded-full self-center mx-auto mb-5 bg-orange-800/50"></div>
          <div class="flex justify-center mb-4">
            <div class="w-14 h-14 rounded-2xl flex items-center justify-center"
              style="background:rgba(251,146,60,0.12);border:1px solid rgba(251,146,60,0.25)">
              <WifiOff :size="24" class="text-orange-400" :stroke-width="1.8" />
            </div>
          </div>
          <p class="text-[18px] font-black text-center mb-2 text-zinc-100">Reset Daemon?</p>
          <p class="text-[13px] text-center leading-relaxed mb-6 text-zinc-500">
            This disconnects and removes the saved daemon pairing. Run <span
              class="text-orange-400 font-bold">orb-daemon pair</span> to reconnect.
          </p>
          <button @click="doResetDaemon" class="w-full py-4 rounded-2xl text-[16px] font-black active:scale-[0.98] mb-3"
            style="background:rgba(251,146,60,0.85);color:white;">
            Reset Pairing
          </button>
          <button @click="showResetDaemon = false"
            class="w-full py-3.5 rounded-2xl text-[15px] font-bold active:scale-[0.98] bg-purple-900/30 text-zinc-400">
            Cancel
          </button>
        </div>
      </div>
    ...
    <Transition name="fade">
      <div v-if="dangerTarget" class="fixed inset-0 z-[300] flex items-end justify-center"
        ...
        <div class="w-full max-w-[430px] rounded-t-[28px] border-t pb-10 px-5 pt-5 bg-[#0e0b1e] border-purple-900/50">
          ...
          <p class="text-[13px] text-center leading-relaxed mb-6 text-purple-400/70">This will permanently erase all
            data. Cannot be undone.</p>
        ...
        <div
          ...
          <div class="flex flex-col gap-4 px-5 pt-4">
            ...
            <div class="grid grid-cols-3 gap-2.5">
              <button v-for="k in ['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0', '⌫']" :key="k + '-s'"
                @click="handleSetupKey(k)"
      ...
      <div v-if="showQuestionSetup" class="fixed inset-0 z-[300] flex items-end justify-center"
        ...
        <div
          ...
          <div class="flex flex-col gap-4 px-5 pt-4 max-h-[88vh] overflow-y-auto pb-2">
            ...
            <p class="text-[13px] text-slate-400 dark:text-zinc-500 text-center -mt-2">Used to reset your PIN if
              forgotten</p>
            <div class="flex flex-col gap-1.5">
              <button v-for="q in securityQuestions" :key="q" @click="selectedQuestion = q" :class="['text-left px-4 py-3 rounded-xl text-[13px] font-semibold border-2 transition-all active:scale-[0.98]',
                selectedQuestion === q
      ...
      <div v-if="showPinManage" class="fixed inset-0 z-[300] flex items-end justify-center"
        ...
        <div
          ...
          <div class="flex flex-col gap-3 px-5 pt-4 pb-2">
            ...
            <button @click="startChangePIN"
              class="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-slate-50 dark:bg-zinc-800 active:scale-[0.98] transition-all">
              <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
                :style="{ background: `${accent}18` }">
                <KeyRound :size="19" :style="{ color: accent }" :stroke-width="1.8" />
              </div>
            ...
            <button @click="showRemovePinConfirm = true; showPinManage = false"
              ...
              <div
                class="w-11 h-11 rounded-2xl bg-rose-100 dark:bg-rose-900/40 flex items-center justify-center flex-shrink-0">
            ...
            <button @click="showPinManage = false"
              class="w-full py-3.5 rounded-2xl bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 text-[15px] font-bold active:scale-[0.98]">Cancel</button>
          </div>
        </div>
      </div>
    </Transition>
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showRemovePinConfirm" class="fixed inset-0 z-[400] flex items-end justify-center"
        ...
        <div
          class="w-full max-w-[430px] bg-white dark:bg-zinc-900 rounded-t-[28px] border-t border-rose-200/60 dark:border-rose-900/40 pb-10 px-5 pt-5">
          ...
          <button @click="doRemovePin"
            class="w-full py-4 rounded-2xl bg-rose-500 text-white text-[16px] font-black active:scale-[0.98] shadow-lg shadow-rose-500/30 mb-3">Yes,
            Remove PIN</button>
          <button @click="showRemovePinConfirm = false"
            class="w-full py-3.5 rounded-2xl bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-300 text-[15px] font-bold active:scale-[0.98]">Cancel</button>
        </div>
      </div>
    </Transition>
        ...
        <div
          ...
          <div class="flex flex-col gap-4 px-5 pt-4">
            ...
            <div v-if="exportStep === 'password'" class="flex flex-col gap-3">
              <div
                :class="['flex items-center gap-3 rounded-2xl px-4 py-3.5 border-2 transition-colors',
                ...
                <Lock :size="17" :class="exportError ? 'text-rose-400' : 'text-slate-400 dark:text-zinc-500'"
            ...
            <div v-else class="flex flex-col items-center gap-4 py-4">
              ...
              <p class="text-[13px] text-slate-400 dark:text-zinc-500 text-center">Your encrypted backup file has been
                downloaded. Keep it and your password safe.</p>
              <button @click="closeExport"
                class="w-full py-4 rounded-2xl text-white text-[16px] font-black active:scale-[0.98]"
      ...
      <div v-if="showImport" class="fixed inset-0 z-[300] flex items-end justify-center"
        ...
        <div
          ...
          <div class="flex flex-col gap-4 px-5 pt-4">
            ...
            <p class="text-[13px] text-slate-400 dark:text-zinc-500 text-center -mt-2 leading-relaxed">
              Select your <span class="font-bold text-slate-600 dark:text-zinc-300">.orbbackup</span> file and enter the
              password used when exporting.
            </p>
            <div v-if="importStep === 'file'" class="flex flex-col gap-3">
              <div
                :class="['flex items-center gap-3 rounded-2xl px-4 py-3.5 border-2 transition-all cursor-pointer',
                ...
                <FileArchive :size="18" :class="importFile ? 'text-violet-500' : 'text-slate-400 dark:text-zinc-600'"
                  :stroke-width="1.8" class="flex-shrink-0" />
                <div class="flex-1 min-w-0">
                  ...
                  <p v-if="importFile" class="text-[10px] text-slate-400 dark:text-zinc-500 mt-0.5">{{
                    formatFileSize(importFile.size) }}</p>
                </div>
              </div>
              ...
              <div
                ...
                <Lock :size="17" :class="importError ? 'text-rose-400' : 'text-slate-400 dark:text-zinc-500'"
                  :stroke-width="2" class="flex-shrink-0" />
              ...
              <div
                class="flex items-start gap-2.5 p-3.5 rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800/40">
            ...
            <div v-else class="flex flex-col items-center gap-4 py-4">
              ...
              <p class="text-[13px] text-slate-400 dark:text-zinc-500 text-center">All your data has been restored. The
                app will reload now.</p>
            </div>
          </div>
        </div>
      </div>
...
const { unpair: daemonUnpair } = useDaemon()
const { navigate } = useNav()
const { mode, setMode } = useDark()
const accent = computed(() => settings.value.accentColor)
const showResetDaemon = ref(false)

function doResetDaemon() {
  daemonUnpair()
  showResetDaemon.value = false
  orbLog('Daemon pairing reset from settings')
}
