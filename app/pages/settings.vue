<template>
  <div class="bg-slate-100 dark:bg-zinc-950 pb-28" style="touch-action:pan-y;">
      ...
      <div class="border-t border-slate-100 dark:border-zinc-800/60 px-4 py-3.5">
        ...
        <div class="grid grid-cols-2 gap-2.5">
          <button v-for="style in cardStyles" :key="style.value" @click="saveSettings({ balanceStyle: style.value })"
            ...
            <div class="w-full rounded-xl overflow-hidden mb-2 relative" style="height:64px;" :style="style.previewStyle(accent)">
              ...
              <div class="absolute flex items-center justify-center" :style="style.orbStyle(accent)">
                ...
                <div class="absolute rounded-full" :style="{ inset: '-3px', border: `1px solid ${accent}55`, borderRadius: '50%' }"></div>
                <div class="absolute inset-0 rounded-full" style="background:radial-gradient(circle at 36% 30%, #1e1b4b 0%, #09090b 55%, #000 100%);"
                  :style="{ boxShadow: `inset 0 0 10px rgba(0,0,0,1), 0 0 0 1px ${accent}44` }"></div>
                <div class="absolute inset-0 rounded-full" :style="{ background: `radial-gradient(circle at 30% 28%, ${accent}30 0%, transparent 55%)` }"></div>
              </div>
    ...
    <!-- CONNECTIONS -->
    <div class="px-5 pb-2">
      <h3 class="text-[11px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Connections</h3>
    </div>
    <div class="mx-4 mb-4 rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-slate-200/60 dark:border-zinc-800/60 shadow-sm">
      <div class="px-4 py-3.5">
        <div class="flex items-center gap-3 mb-4">
          <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0" :style="{ background: `${accent}18` }">
            <Monitor :size="19" :style="{ color: accent }" :stroke-width="1.8" />
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-[14px] font-bold text-slate-800 dark:text-zinc-100">Max Connected Devices</p>
            <p class="text-[11px] text-slate-400 dark:text-zinc-500 mt-0.5">
              Maximum simultaneous daemon connections
            </p>
          </div>
          <div class="flex-shrink-0 px-3 py-1.5 rounded-xl"
            :style="{ background: `${accent}18`, border: `1px solid ${accent}30` }">
            <span class="text-[16px] font-black font-mono" :style="{ color: accent }">{{ settings.maxConnectedDevices }}</span>
          </div>
        </div>

        <!-- Slider row -->
        <div class="flex items-center gap-3">
          <span class="text-[11px] font-mono font-bold text-slate-400 dark:text-zinc-600 w-4 text-center">1</span>
          <div class="flex-1 flex gap-1.5">
            <button
              v-for="n in 10" :key="n"
              @click="saveSettings({ maxConnectedDevices: n })"
              class="flex-1 h-8 rounded-lg transition-all active:scale-95 flex items-end justify-center pb-1"
              :style="n <= settings.maxConnectedDevices
                ? { background: accent, boxShadow: `0 2px 8px ${accent}44` }
                : { background: 'rgba(0,0,0,0.08)' }"
              :class="n <= settings.maxConnectedDevices ? '' : 'dark:bg-zinc-800'">
              <span class="text-[8px] font-black"
                :style="n <= settings.maxConnectedDevices ? { color: '#fff' } : { color: '#a1a1aa' }">
                {{ n }}
              </span>
            </button>
          </div>
          <span class="text-[11px] font-mono font-bold text-slate-400 dark:text-zinc-600 w-5 text-center">10</span>
        </div>

        <p class="text-[11px] text-slate-400 dark:text-zinc-600 mt-3 leading-relaxed">
          Currently <span class="font-bold" :style="{ color: accent }">{{ settings.maxConnectedDevices }} device{{ settings.maxConnectedDevices !== 1 ? 's' : '' }}</span> allowed.
    ...
    <!-- ADVANCED -->
    <div class="px-5 pb-2">
      <h3 class="text-[11px] font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-widest">Advanced</h3>
    </div>
    <div class="mx-4 mb-4 rounded-2xl overflow-hidden bg-white/70 dark:bg-zinc-900/60 backdrop-blur border border-slate-200/60 dark:border-zinc-800/60 shadow-sm">
      ...
      <button @click="navigate('developer')"
        ...
        <div class="flex-1 min-w-0">
          ...
          <p class="text-[11px] text-slate-400 dark:text-zinc-500 mt-0.5">Logs, diagnostics, reset &amp; perf monitor</p>
        </div>
        <ChevronRight :size="17" class="text-slate-300 dark:text-zinc-700 flex-shrink-0" :stroke-width="2" />
      </button>
      ...
      <button @click="showResetDaemon = true"
        class="w-full flex items-center gap-3 px-4 py-3.5 active:bg-orange-50 dark:active:bg-orange-950/20 transition-colors">
        <div class="w-11 h-11 rounded-2xl flex items-center justify-center flex-shrink-0"
  ...
  <Teleport to="body">
    <Transition name="fade">
      <div v-if="showResetDaemon" class="fixed inset-0 z-[300] flex items-end justify-center"
        ...
        <div class="w-full max-w-[430px] bg-[#0e0b1e] rounded-t-[28px] border-t border-orange-900/40 pb-10 px-5 pt-5">
          ...
          <p class="text-[13px] text-center leading-relaxed mb-6 text-zinc-500">
            This disconnects and removes the saved daemon pairing. Run <span class="text-orange-400 font-bold">orb-daemon pair</span> to reconnect.
          </p>
      ...
      <div v-if="showRemovePinConfirm" class="fixed inset-0 z-[400] flex items-end justify-center"
        ...
        <div class="w-full max-w-[430px] bg-white dark:bg-zinc-900 rounded-t-[28px] border-t border-rose-200/60 dark:border-rose-900/40 pb-10 px-5 pt-5">
          ...
          <button @click="doRemovePin"
            class="w-full py-4 rounded-2xl bg-rose-500 text-white text-[16px] font-black active:scale-[0.98] shadow-lg shadow-rose-500/30 mb-3">Yes, Remove PIN</button>
        ...
        <div class="w-full max-w-[430px] bg-white dark:bg-zinc-900 rounded-t-[28px] border-t border-slate-200/60 dark:border-zinc-800"
          ...
          <div class="flex flex-col gap-4 px-5 pt-4">
            ...
            <p class="text-[13px] text-slate-400 dark:text-zinc-500 text-center -mt-2 leading-relaxed">
              Your data will be encrypted with a password. You'll need this password to import on another device.
            </p>
            ...
            <div v-else class="flex flex-col items-center gap-4 py-4">
              ...
              <p class="text-[13px] text-slate-400 dark:text-zinc-500 text-center">Your encrypted backup file has been downloaded.</p>
...
import {
  ...
  AlertTriangle, CheckCircle, FlaskConical, WifiOff,
...
const cardStyles = [
  {
    value: 'supreme' as const, label: 'Supreme Orb', sub: 'Centered with starfield', orbLabel: 'orb · center',
    ...
    orbStyle:     (_a: string) => ({ width: '30px', height: '30px', top: '50%', left: '50%', transform: 'translate(-50%, -58%)' }),
  },
  {
    value: 'minimal' as const, label: 'Minimal Orb', sub: 'Compact, left-aligned', orbLabel: 'orb · left',
    ...
    orbStyle:     (_a: string) => ({ width: '26px', height: '26px', top: '50%', left: '14px', transform: 'translateY(-50%)' }),
  },
  {
    value: 'neon' as const, label: 'Neon Card', sub: 'Centered with edge glow', orbLabel: 'orb · center',
    ...
    orbStyle:     (_a: string) => ({ width: '30px', height: '30px', top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }),
  },
  {
    value: 'glass' as const, label: 'Glass Card', sub: 'Frosted, orb top-center', orbLabel: 'orb · top',
    ...
    orbStyle:     (_a: string) => ({ width: '26px', height: '26px', top: '10px', left: '50%', transform: 'translateX(-50%)' }),
  },
]
...
async function doExport() {
  ...
  try {
    ...
    exportStep.value = 'done'; orbLog('Data exported successfully')
  } catch (e: any) {
...
async function doImport() {
  ...
  try {
    ...
    importStep.value = 'done'; orbLog('Data imported successfully')
